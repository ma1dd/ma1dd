...
const CreateSession = () => {
  ...
  useEffect(() => {
    fetch('/data/products.json')
      ...
      .catch((error) => {
        ...
        setStatus({ type: 'error', message: 'Не удалось загрузить список товаров' });
      });
  }, []);
  ...
  useEffect(() => {
    ...
    const prefetchedProduct = products.find((product) => String(product.id) === preselected);
    ...
    setTimeout(() => {
      ...
      setProductRows((prev) => prev.map((row, idx) => {
        ...
        const exactMatch = products.find(
          (product) => product.название.toLowerCase() === row.query.trim().toLowerCase()
        );
        return exactMatch ? { ...row, productId: exactMatch.id, query: exactMatch.название } : { ...row, productId: row.productId };
      }));
    }, 200);
  };
  ...
  const getFilteredProducts = (row) => {
    ...
    return products.filter((product) => product.название.toLowerCase().includes(queryLower)).slice(0, 5);
  };

  const selectedProductDetails = useMemo(() => {
    return productRows
      .map((row) => products.find((product) => String(product.id) === String(row.productId)))
      .filter(Boolean);
  ...
  const handleSubmit = (event) => {
    ...
    if (filledProducts.length < MIN_PRODUCTS) {
      setStatus({ type: 'error', message: 'Не удалось собрать данные по выбранным товарам' });
      return;
    }

    if (!currentUser) {
      setStatus({ type: 'error', message: 'Не удалось определить пользователя' });
      return;
    }
    ...
    try {
      ...
      setStatus({ type: 'success', message: 'Аналитическая сессия сохранена! Перенаправляем на страницу анализа...' });
      setTimeout(() => {
      ...
      setStatus({ type: 'error', message: 'Не получилось сохранить сессию. Попробуйте ещё раз.' });
      ...
      <div className="create-session-header">
        <div>
          ...
          <p className="create-session-subtitle">
            Выберите минимум два товара, задайте период наблюдения и зафиксируйте гипотезы. После подтверждения мы построим сравнительную
            аналитику по каждому параметру.
          </p>
        </div>
        <div className="create-session-owner">
          <img
            src={currentUser?.аватар || '/data/img/avatars/stock_user_avatar.png'}
            alt={formatUserName()}
            onError={(e) => {
      ...
      <div className="create-session-grid">
        <form className="create-session-form" onSubmit={handleSubmit}>
          <section className="form-section">
            ...
            <div className="form-group">
              <label htmlFor="session-description">Описание / цель исследования</label>
              <textarea
            ...
            <div className="form-group">
              <label htmlFor="session-thoughts">Мысли, гипотезы, ожидания</label>
              <textarea
                id="session-thoughts"
                rows="3"
          ...
          <section className="form-section">
            ...
            <p className="section-hint">Минимум два товара. Можно добавить любые позиции позднее прямо со страницы продукта.</p>
            {productRows.map((row, index) => {
              const suggestions = getFilteredProducts(row);
              return (
                <div key={`product-select-${index}`} className="product-select-row single-line">
                  <div className="product-select-group product-text-search">
                    ...
                    <input
                      ...
                      autoComplete="off"
                    />
                    {suggestions.length > 0 && row.query && showSuggestions[index] && (
            ...
            <button type="button" className="btn-secondary" onClick={handleAddProductField}>
              + добавить товар
            </button>
          </section>
          ...
          <div className="create-session-actions">
            <button type="submit" className="btn-primary" disabled={isSubmitting}>
              {isSubmitting ? 'создаём...' : 'подтвердить и перейти к аналитике'}
            </button>
            <button
              type="button"
              className="btn-ghost"
        ...
        <aside className="session-preview-panel">
          ...
          {selectedProductDetails.length === 0 ? (
            <p className="preview-placeholder">
              Выберите товары, чтобы увидеть ключевые метрики перед запуском анализа.
            </p>
          ) : (
            <ul className="preview-list">
              {selectedProductDetails.map((product) => {
                ...
                const positivePercent = reviews > 0 ? Math.round((positive / reviews) * 100) : 0;
                return (
                  <li key={product.id} className="preview-item">
                    <div className="preview-item-header">
                      ...
                      <div>
                        <strong>{product.название}</strong>
                        <span className="preview-category">{product.категория?.название || product.категория}</span>
                      </div>
                    </div>
