...
import { addProductToSession, loadCustomSessions } from '../utils/sessionsStorage';
...
const ProductDetail = () => {
  ...
  const handleAddToSession = () => {
    if (!selectedSessionId) {
      setStatus({ type: 'error', message: 'Выберите сессию для добавления товара' });
      return;
    }
    if (!product) {
      return;
    ...
    if (result.reason === 'session-not-found') {
      setStatus({ type: 'error', message: 'Сессия не найдена. Обновите страницу.' });
      setSessions(loadCustomSessions());
      return;
    }
    ...
    setStatus({ type: 'error', message: 'Не удалось добавить товар. Попробуйте позже.' });
  };

  if (loading) {
    return <div className="product-detail-loading">Загружаем данные о товаре...</div>;
  }
  ...
  if (error || !product) {
    return (
      <div className="product-detail-error">
        ...
        <button className="btn-primary" onClick={() => navigate('/products')}>Вернуться к каталогу</button>
      </div>
    );
  }
  ...
  return (
    <div className="product-detail-content">
      <button className="btn-ghost" onClick={() => navigate(-1)}>← Назад к каталогу</button>
      ...
      <div className="product-detail-header">
        ...
        <div className="product-detail-price">
          ...
          <div className="product-price-value">
            <span className="product-price-amount">{product.цена?.toLocaleString('ru-RU')}</span>
      ...
      <section className="product-detail-section">
        ...
        <div className="product-detail-stats">
          <div className="stat-card">
            ...
            <strong>{product.статистика_отзывов?.всего_отзывов?.toLocaleString('ru-RU')}</strong>
          </div>
          <div className="stat-card">
            <span>Средний рейтинг</span>
            <strong>{product.статистика_отзывов?.средний_рейтинг?.toFixed(1)} / 5</strong>
          </div>
          <div className="stat-card">
      ...
      <section className="product-detail-section">
        <div className="section-heading">
          ...
          <button className="btn-link" onClick={() => navigate('/create_session', { state: { preselectedProductId: product.id } })}>
        ...
        {sessions.length ? (
          <div className="add-to-session-form">
            ...
            <button className="btn-primary" type="button" onClick={handleAddToSession}>
              добавить к сессии
            </button>
          </div>
        ) : (
          <div className="product-detail-empty">
            <p>У вас пока нет пользовательских сессий. Создайте первую, чтобы добавлять товары в сравнение.</p>
            <button
              className="btn-secondary"
              type="button"
              onClick={() => navigate('/create_session', { state: { preselectedProductId: product.id } })}
            >
              создать сессию
            </button>
          </div>
