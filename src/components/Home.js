...
const Home = () => {
  ...
  const chartData = React.useMemo(() => generateChartData(periodDays), [periodDays]);
  ...
  const handlePeriodChange = () => {
    const newPeriod = prompt('Введите количество дней (от 1 до 30):', periodDays);
    ...
    fetch('/data/products.json')
      ...
      .then(data => {
        ...
        data.forEach(product => {
          ...
          totalRating += product.статистика_отзывов.средний_рейтинг * product.статистика_отзывов.всего_отзывов;
        ...
        const positivePercent = totalReviews > 0 ? Math.round((totalPositive / totalReviews) * 100) : 0;
  ...
  return (
    <div className="home-content">
        ...
        <div className="widget">
          ...
          <div className="chart-controls">
            <div className="period-selector">
              ...
              <span className="period-active" onClick={handlePeriodChange} style={{ cursor: 'pointer' }}>{periodDays}</span>
              <span> дней</span>
            </div>
            <button className="btn-details">подробнее</button>
          </div>
      ...
      <div className="sessions-widget">
              ...
              displayedSessions.map(session => (
                <Link key={session.id} to={`/sessions/${session.id}`} className="table-row session-row-link">
                  ...
                  <div className="table-cell table-cell-analysis">
                    <strong className="session-title-text">{session.title || session.название || session.анализ}</strong>
                    <p className="session-summary">{session.description || session.анализ}</p>
                  </div>
        ...
        {displayedSessions.length > 0 && (
          hasMore ? (
            <button className="btn-load-more" onClick={loadMoreSessions}>загрузить ещё</button>
          ) : (
