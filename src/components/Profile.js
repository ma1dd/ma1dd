...
const Profile = () => {
        ...
        const sessions = [...loadCustomSessions(), ...sessionsData]
          ...
          .map(session => ({
            ...
            пользователь: {
              ...
              аватар: profileInfo.аватар || '/data/img/avatars/stock_user_avatar.png',
            }
          }))
  ...
  useEffect(() => {
    ...
    if (searchText) {
      ...
      result = result.filter(session => {
        ...
        if (session.description?.toLowerCase().includes(searchLower)) return true;
    ...
    if (filters.role) {
      result = result.filter(session => session.пользователь.роль === filters.role);
    }
    ...
    if (filters.dateRange) {
      ...
      if (cutoffDate && filters.dateRange !== 'custom') {
        result = result.filter(session => new Date(session.updatedAt || session.createdAt) >= cutoffDate);
      }
    }
  ...
  if (profileError) {
    return (
      <div className="session-detail-error">
        <p>{profileError}</p>
        <Link to="/" className="btn-primary">Перейти на главную</Link>
      </div>
    );
  }
  ...
  const handleLoadMore = () => {
    ...
    const nextBatch = filteredSessions.slice(currentCount, currentCount + itemsPerPage);
      ...
      <div className="profile-card">
        <div className="profile-header">
          <div className="profile-avatar-container">
            <img
              src={profileUser.аватар || '/data/img/avatars/stock_user_avatar.png'}
          ...
          <div className="profile-info">
            <h1>{profileUser.фамилия} {profileUser.имя} {profileUser.отчество}</h1>
            <p className="role">{profileUser.роль}</p>
            <p className="status">В сети: {profileUser.в_сети || 'неизвестно'}</p>
          ...
          {isOwnProfile && (
            <div className="profile-actions">
              <Link to="/edit_profile" className="btn-edit">Изменить профиль</Link>
            </div>
          )}
        </div>
      </div>
              ...
              displayedSessions.map(session => (
                <Link key={session.id} to={`/sessions/${session.id}`} className="table-row session-row-link">
                  <div className="table-cell table-cell-analysis">
                    <strong className="session-title-text">{session.title || 'Аналитическая сессия'}</strong>
                    <p className="session-summary">{session.description || session.comment || 'описание отсутствует'}</p>
                  </div>
