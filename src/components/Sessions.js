...
const Sessions = () => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([sessionsData, usersData]) => {
        ...
        const combined = [...customSessions, ...updatedSessions].sort((a, b) => {
  ...
  useEffect(() => {
    ...
    if (searchText) {
      ...
      result = result.filter(s => {
        ...
        if (s.пользователь.полное_имя?.toLowerCase().includes(searchLower)) return true;
        if (s.пользователь.фамилия?.toLowerCase().includes(searchLower)) return true;
        if (s.пользователь.имя_полное?.toLowerCase().includes(searchLower)) return true;
        if (s.пользователь.отчество?.toLowerCase().includes(searchLower)) return true;
        
        // Поиск по роли
        if (s.пользователь.роль?.toLowerCase().includes(searchLower)) return true;
    ...
    if (filters.dateRange) {
      ...
      if (cutoffDate && filters.dateRange !== 'custom') {
        result = result.filter(s => new Date(s.updatedAt || s.createdAt) >= cutoffDate);
      }
    }
  ...
  const handleLoadMore = () => {
    ...
    const nextSessions = filteredSessions.slice(currentCount, currentCount + itemsPerPage);
  ...
  const getSessionSummary = (session) => session.description || session.comment || 'описание отсутствует';
          ...
          <div className="search-filter-bar">
            <div className="search-input-wrapper">
              ...
              <button className="btn-search" onClick={handleSearch}>
                <img src="/data/img/serach icon.png" alt="search" />
              </button>
            </div>
            <button className="btn-filter" onClick={() => setShowFilters(true)}>фильтры</button>
            <Link to="/create_session" className="btn-create-session">создать сессию</Link>
          </div>
        </div>
          ...
          <div className="table-body">
            {displayedSessions.length > 0 ? (
              displayedSessions.map(session => (
                <Link key={session.id} to={`/sessions/${session.id}`} className="table-row session-row-link">
                  <div className="table-cell table-cell-user">
                    ...
                    />
                    <span>{session.пользователь.имя} ({session.пользователь.роль})</span>
                  </div>
                  <div className="table-cell table-cell-analysis">
                    <strong className="session-title-text">{getSessionTitle(session)}</strong>
                    <p className="session-summary">{getSessionSummary(session)}</p>
                    ...
                    {getPeriodLabel(session) && (
                      <span className="session-period">{getPeriodLabel(session)}</span>
                    )}
                  </div>
              ...
              <div className="table-row no-results-row">
                <div className="table-cell" style={{ gridColumn: '1 / -1' }}>Сессии не найдены.</div>
              </div>
            )}
          </div>
        </div>
